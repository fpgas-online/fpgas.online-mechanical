...
def _arty_targets(spec: BoardSpec) -> dict[str, Target]:
    ...
    return {
        ...
        "eth": Target(
            "eth", "LD0-LD7 and the RJ45", ex0, ey0, ex1, ey1,
...
def _acorn_subject() -> Subject:
    ...
    return Subject(
        ...
        spec=spec,
        targets=(
            Target("assembly", "The whole assembly", ex0, ey0, ex1, ey1,
            ...
            Target("card", "The Acorn card", card.x0, card.y0, card.x1,
                   card.y1,
        ...
        sources=(
            Source(label="Board geometry", ref="raspberry_pi/boards.py",
                   note="Raspberry Pi 5 outline and connectors, from "
                        "Raspberry Pi Ltd's mechanical drawing; see "
            ...
            Source(label="PCI Express M.2 Specification",
                   ...
                   note="Section 2.3.4.3 and figure 13: a Type 2280 module "
                        "is 22 +/-0.15 by 80 +/-0.15 mm."),
            Source(label="SQRL Acorn CLE-215+ product page",
                   ...
                   note='Squirrels Research Labs, captured 2020; the site is '
                        'gone. Quoted: "it is one millimeter wider than the '
            ...
            Source(label="Waveshare PoE M.2 HAT+ (B) dimension drawing",
                   ...
                   note='Annotated 85.00, 56.00 and 3.00, "Unit: mm"; the '
                        "3.00 is the 2280 standoff's projection past the "
                        "board edge. The card's seated position is DERIVED "
                        "from this drawing in accessories/parts.py, +/-0.2 "
                        f"mm, and {hat_sheet} is drawn from the same "
                        "figures."),
        ),
        notes=(
            "The Acorn's own LEDs are not published: SQRL issued no "
            "mechanical drawing and their site is gone. No indicator frame "
            "is drawn for it. The second frame here is the card, not its "
            "LEDs.",
            "The heatsink the CLE-215+ carries is not published either, so "
            "no height above the card is claimed and the camera height is "
            "measured to the Pi's own top face.",
            "The seated card position and the assembly's far edge are "
            f"imported from accessories/parts.py, which {hat_sheet} is drawn "
            "from as well, so the card cannot be in two places.",
        ),
    )

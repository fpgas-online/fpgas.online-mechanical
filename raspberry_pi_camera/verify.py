...
* **Quotes.**  Every figure and phrase ``v1.py`` attributes to a source is
  ...
  article and Raspberry Pi's documentation as pages.
...
* **The independent checks**, which are what the error bars rest on.  The
  hole pattern Gert measured by hand in 2013 against the one Raspberry Pi's
  own Camera Module 2 and 3 drawings give; his three heights added up
  against Raspberry Pi's "around 9 mm"; Raspberry Pi Spy's second measurement
  against his.  Each has a limit, and the limit is the figure the sheet
  quotes.
...
THREAD_PHRASES = (
    ...
    "the lens axis is just off line from the adjacent mounting holes",
...
SPY_PHRASES = ("The PCB is 25x24mm and is approximately 1mm thick.",
               ...
               "will accept a 2mm machine screw",
...
#: The limits the sheet quotes, and so the limits held here.
HOLE_PATTERN_LIMIT = 0.03
HEIGHT_LIMIT = 0.1          # RPI says "around" 9; the three add to 8.95
...
def arithmetic() -> None:
    ...
    check(math.isclose(v1.FFC[1], h - 5.6, abs_tol=0.005)
          ...
          "connector is 5.6 deep and 2.8 below the board, cable 1.27 below")
    ...
    c0, c1 = m["connector_v"]
    check(math.isclose(v1.FFC[0], round(c0, 2), abs_tol=0.005)
          and math.isclose(v1.FFC[2], round(c1, 2), abs_tol=0.005),
          f"connector along the edge is the scaled {c0:.2f} to {c1:.2f}")
    t0, t1 = m["tail_u"]
...
def independent() -> None:
    ...
    # Raspberry Pi Spy, measured with calipers, the same weeks.
    ...
    check(round(abs(spy_lens - gvl_lens), 2) <= v1.LENS_TOL,
          f"lens module {gvl_lens:.2f} from the connector edge, Raspberry Pi "
          f"Spy's {spy_lens}: {abs(spy_lens - gvl_lens):.2f} apart, "
          f"within LENS_TOL {v1.LENS_TOL}")
    # For information, not held to anything: a different board's connector.
    body = 19.61
    along = v1.FFC[2] - v1.FFC[0]
    print(f"  info  connector {along:.2f} along the edge; the Camera Module "
          f"2 and 3 bodies are {body} on Raspberry Pi's drawings")

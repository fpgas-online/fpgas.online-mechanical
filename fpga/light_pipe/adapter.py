...
KEYWAY_Y, KEYWAY_Z0, KEYWAY_Z1 = 3.32, 10.64, 12.461
...
BORE_ANGLE = 45.0
BORE_Y = 6.48
BORE_X, BORE_Z = -2.184, 12.08
BORE_DIA = 3.2
PRESS_DIA, PRESS_LEN = 2.92, 2.0
ENTRY_K = 14.264
FACET_K = 19.921
FACET_X, FACET_Z = -5.012, 14.908

#: The cheek: the block the bore runs through, cut off at the facet.
CHEEK_X0, CHEEK_X1 = -6.427, 0.0
CHEEK_Y0, CHEEK_Y1 = 4.08, 9.99
...
ADAPTER = BoardSpec(
    ...
    body="enclosure",
    front_edge="left",
    outline=Outline(width=16.427, height=19.98, z_height=9.427),
    ...
    sources=(
        ...
        Source(label="Board drawing",
               ...
               note="Digilent, Arty_A7_DXF.DXF: board locks 16.104 apart against Bel's 16.13; centre plane y = 44.0, and through Bel's 7.75 the front face x = -0.101 +/-0.628, which is how far Digilent's own plot of the jack disagrees."),
    ...
    notes=(
        "J9's LED windows stand 0.64 mm proud of the face the cheeks land on, so the pipe tips are set from the window, at maximum material.",
        "Nothing holds the adapter on but J9's own side EMI springs, 1.4 +/-0.51 proud of a 16.31 +/-0.254 shield, bearing on the skirts: deflected 0.33 mm at minimum material and 1.6 at maximum, the shield never touching.",
        ...
        "STATED is Bel's own dimension, +/-0.254; MEASURED is read off its geometry, +/-0.20; DERIVED is worked out from those and Bivar's pipe.",
    ),
)

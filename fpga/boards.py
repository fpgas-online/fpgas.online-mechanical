...
BOARDS['icepi-zero'] = BoardSpec(
    ...
    sources=(
        ...
        Source(label='Board revision',
               ...
               note='the v1.3 tag, "Final mass production files": the revision made in quantity and sold. The same file at the tip of the repository, d67bb758, is read too and agrees on every position drawn here.'),
        Source(label='Board size',
               ref='https://github.com/cheyao/icepi-zero/blob/v1.3/README.md',
               note='quoted: "The Icepi Zero is an FPGA development board in the popular Raspberry Pi Zero form factor"; the board file gives 65.00 x 30.00 mm.'),
    ...
    notes=(
        'Hole IDs are the KiCad reference designators.',
        "The mounting holes are the Raspberry Pi Zero pattern: 58.00 x 23.00 mm, 3.50 mm in from each edge of a 65 x 30 board. They are drilled 2.70 where the Pi Zero drawing says 2.75 +/-0.05, and the corners are R3.50 where the Pi Zero's are R3.00.",
        ...
        'The board file has been re-annotated since the mass-production release. Nothing drawn here moved, but in the current file the programming port is J5 and the user LEDs run D1 to D5 left to right; these are the designators the sold boards were fabbed with.',
    ),
)

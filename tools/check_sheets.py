...
def check_drawing_names() -> list[str]:
    ...
    minimum, the widest name in the set; the light pipe's sheet would
    have been ``FPGA-ARTY-ETHERNET-LIGHT-PIPE`` at 58.03 mm had it not
    been given a rule of its own, and is ``FPGA-LP-ARTY`` at 24.97 mm.
    Every name on a sheet with a title block fits the

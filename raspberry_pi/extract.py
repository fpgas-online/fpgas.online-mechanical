#!/usr/bin/env python3
"""Generate ``raspberry_pi/boards.py`` from Raspberry Pi Ltd drawings.
...
Fetch them with ``tools/fetch_raspberry_pi.sh``.
...
from tools.dump_rpi_pdf import find_origin, points_mm, rectangles  # noqa: E402
RPI = ROOT / "tmp" / "rpi"
...
def extract(model: dict) -> dict:
    path = RPI / model["file"]
    if not path.exists():
        raise SystemExit(f"missing source {path}; run tools/fetch_raspberry_pi.sh")
...
def main() -> None:
    ...
    out = ROOT / "raspberry_pi" / "boards.py"
